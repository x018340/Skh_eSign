        ...
        for _, m in display_results.iterrows():
            ...
            with st.expander(title_str):
                ...
                with r2:
                    ...
                    qr_bytes = generate_qr_card(
                        m_url, str(m_name), str(m.get("Location")), str(m.get("TimeRange"))
                    )
                        ...
                        if st.button("📄 Generate PDF", key=f"gen_{m_id}"):
                            ...
                            with st.spinner("Generating..."):
                                if "RankID" in fresh_att_subset.columns:
                                    fresh_att_subset["RankID_Int"] = fresh_att_subset["RankID"].apply(
                                ...
                                display_time = (
                                    f"時間：{t_range}"
                                    if "/" in t_range
                                    else f"時間：{str(fresh_m.get('MeetingDate')).replace('-', '/')} {t_range}"
                                )
                                ...
                                for i, row in fresh_att_subset.reset_index().iterrows():
                                    ...
                                    img = image_from_signature_value(sig_val, drive_service=drive_service)
                                    if img is not None:
                                ...
                                # Safer bytes output across fpdf variants
                                try:
    ...
    # --- 3. EMPLOYEE MASTER ---
        ...
        if st.session_state.df_master is not None:
            edited_df = st.data_editor(
                st.session_state.df_master, num_rows="dynamic", use_container_width=True, height=600
            )

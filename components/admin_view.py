...
# ---- Option A time sync helpers (Start + Duration <-> End) ----
def _combine(d: date_t, t: time_t) -> datetime:
    return datetime.combine(d, t)


def _clamp_duration(mins: int) -> int:
    return max(0, min(24 * 60, int(mins)))


def _ensure_time_state_defaults():
    if "meeting_date" not in st.session_state:
        st.session_state.meeting_date = datetime.today().date()
    if "start_time" not in st.session_state:
        st.session_state.start_time = datetime.strptime("12:00", "%H:%M").time()
    if "duration_min" not in st.session_state:
        st.session_state.duration_min = 60
    if "end_time" not in st.session_state:
        end_dt = _combine(st.session_state.meeting_date, st.session_state.start_time) + timedelta(
            minutes=st.session_state.duration_min
        )
        st.session_state.end_time = end_dt.time()
    if "last_time_edit" not in st.session_state:
        st.session_state.last_time_edit = "duration"


def _on_change_duration():
    st.session_state.last_time_edit = "duration"
    mins = _clamp_duration(st.session_state.duration_min)
    start_dt = _combine(st.session_state.meeting_date, st.session_state.start_time)
    end_dt = start_dt + timedelta(minutes=mins)
    st.session_state.end_time = end_dt.time()


def _on_change_end():
    st.session_state.last_time_edit = "end"
    start_dt = _combine(st.session_state.meeting_date, st.session_state.start_time)
    end_dt = _combine(st.session_state.meeting_date, st.session_state.end_time)
    if end_dt < start_dt:
        # assume crossing midnight
        end_dt += timedelta(days=1)
    mins = int((end_dt - start_dt).total_seconds() // 60)
    st.session_state.duration_min = _clamp_duration(mins)


def _on_change_start_or_date():
    # Recompute dependent field based on what user last edited
    if st.session_state.get("last_time_edit") == "end":
        _on_change_end()
    else:
        _on_change_duration()
...
def show_admin():
    ...
    if st.sidebar.button("🧪 Test Upload"):
        if not (GAS_UPLOAD_URL and GAS_API_KEY and GAS_FOLDER_ID):
            st.sidebar.error("Set [gas] upload_url, api_key, folder_id in secrets.")
        else:
            try:
    ...
    if menu == "🗓️ Arrange Meeting":
        ...
        if df_master is None or df_master.empty or "FullName" not in df_master.columns:
            st.warning("⚠️ Employee Master not loaded. Please click Refresh in sidebar.")
            return
        ...
        if st.session_state.created_meeting_data:
            ...
            c1, c2 = st.columns(2)
            with c1:
                st.image(card_bytes, caption="Preview", width=250)
            with c2:
        ...
        if "form_name" not in st.session_state:
            st.session_state.form_name = ""
        if "form_loc" not in st.session_state:
            st.session_state.form_loc = ""
        if "form_selected" not in st.session_state:
            st.session_state.form_selected = []

        _ensure_time_state_defaults()

        col1, col2 = st.columns(2)
        name = col1.text_input("Meeting Name", key="form_name")
        loc = col2.text_input("Location", key="form_loc")

        # Date + time inputs with auto-sync
        st.date_input("Meeting Date", key="meeting_date", on_change=_on_change_start_or_date)
        c_t1, c_t2, c_t3 = st.columns(3)
        c_t1.time_input("Start", key="start_time", on_change=_on_change_start_or_date)
        c_t2.number_input("Duration (min)", min_value=0, max_value=24*60, step=5, key="duration_min", on_change=_on_change_duration)
        c_t3.time_input("End", key="end_time", on_change=_on_change_end)
        ...
        if "RankID" in filtered_emp.columns:
            filtered_emp = filtered_emp.copy()
        ...
        if st.session_state.processing_create:
            ...
            max_id = 0
            if df_info_live is not None and not df_info_live.empty and "MeetingID" in df_info_live.columns:
            ...
            end_t = st.session_state.end_time

            date_str = meeting_date.strftime("%Y/%m/%d")
            time_range = f"{date_str} {start_t.strftime('%H:%M')}~{end_t.strftime('%H:%M')}"

            ws_info = get_sheet_object("Meeting_Info")
            ws_info.append_row(
                map_dict_to_row(
                    df_info_live.columns.tolist(),
                    {
                        "MeetingID": new_id,
                        "MeetingName": name,
    ...
    # ---- Meeting Control ----
        ...
        if s_id:
            results = results[results["MeetingID"].astype(str) == s_id]
        if s_date:
            results = results[results["d_obj"] == s_date]
                    ...
                    if pdf_key in st.session_state.pdf_cache:
                        ...
                        if st.button("📄 Generate PDF", key=f"gen_{m_id}"):
                            ...
                            with st.spinner("Generating..."):
                                ...
                                # "blank" look: no gray fill
                                pdf.set_font_size(16)
                                pdf.cell(80, 12, "出席人員", 1, 0, "C", False)
                                pdf.cell(110, 12, "簽名", 1, 1, "C", False)

                                for i, row in fresh_att_subset.reset_index(drop=True).iterrows():
                    ...
                    if new_name_input and new_dept_input:
                        try:
                            ...
                            row = map_dict_to_row(
                                ...
                                {
                                    "RankID": int(new_rank),
                                    "FullName": new_name_input,
                                    "JobTitle": new_job_input,
                                    "Department": new_dept_input,
                                },
                            )

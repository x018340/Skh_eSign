...
import pandas as pd
import gspread
from google.oauth2.service_account import Credentials # CHANGED: Modern Auth
from googleapiclient.discovery import build # ADDED: For Drive
from googleapiclient.http import MediaIoBaseUpload, MediaIoBaseDownload # ADDED: For Drive
...
FONT_CH = "font_CH.ttf"
FONT_EN = "font_EN.ttf"
DEPLOYMENT_URL = st.secrets["general"]["deployment_url"] # CHANGED: Load from secrets
ADMIN_KEY = st.secrets["general"]["admin_password"] # CHANGED: Load from secrets
DRIVE_FOLDER_ID = st.secrets["general"]["drive_folder_id"] # ADDED: Target Folder

st.set_page_config(page_title="SKH E-Sign System", page_icon="✍️", layout="wide")

# ==============================================================================
# 1. CONNECTION (Modified for Drive + Sheets)
# ==============================================================================
@st.cache_resource
def get_services():
    """Returns (Sheet_Client, Drive_Service)"""
    scope = [
    ...
    # CHANGED: Use google-auth (modern)
...
def get_sheet_object(worksheet_name):
    client, _ = get_services() # We only need the sheet client here
    retries = 3
...
# ==============================================================================
# 3. HELPERS (New Drive Helpers Added)
# ==============================================================================
...
# ADDED: Helper to download image for PDF
...
def base64_to_image(base64_str):
    # Still used if any old data remains, but mostly unused now
    try:
                ...
                if is_canvas_blank(canvas.image_data):
                    ...
                    with st.status("🚀 Processing...", expanded=True) as status:
                        try:
                            ...
                            client, drive_service = get_services()
                            ...
                            # 2. Update Sheet (Original Logic logic adapted for File ID)
                            ...
                            if row_update_idx > 0:
                                # 10 RETRIES LOGIC (KEPT AS REQUESTED)
                                retries = 10
                                for i in range(retries):
                                    try:
                                        ws_attendees.batch_update([
                                            {'range': gspread.utils.rowcol_to_a1(row_update_idx, status_idx), 'values': [['Signed']]},
                                            {'range': gspread.utils.rowcol_to_a1(row_update_idx, sig_idx), 'values': [[file_id]]} # SAVING ID
                                        ])
                                        break
                                    except gspread.exceptions.APIError as e:
...
# ------------------------------------------------------------------------------
    ...
    # --- 1. ARRANGE MEETING (KEPT IDENTICAL) ---
    if menu == "🗓️ Arrange Meeting":
        st.title("Arrange New Meeting")
        df_master = st.session_state.df_master
        # ... [Logic identical to original] ...
        # (I am omitting the 100 lines of unchanged UI code for brevity, 
        # but in your file, you KEEP IT EXACTLY THE SAME)
        # Ensure you use 'ADMIN_KEY' variable for check, which now comes from secrets.
        
        # NOTE: Copy the "1. ARRANGE MEETING" block from your original file here.
        # It needs no changes for Drive Integration.

        # --- RE-INSERTING YOUR ORIGINAL ARRANGE MEETING LOGIC ---
    ...
    # --- 2. MEETING CONTROL (Merged) ---
        ...
        for _, m in display_results.iterrows():
            ...
            with st.expander(title_str):
                ...
                with r3:
                    pdf_key = f"pdf_{m_id}"
                    # --- MODIFIED: PDF LOGIC FOR DRIVE DOWNLOAD ---
                    if st.button("📄 Generate PDF", key=f"gen_{m_id}"):
                        ...
                        with st.spinner("Downloading images & Generating PDF..."):
                            ...
                            client, drive_service = get_services() # Get Drive Service
                            ...
                            for i, row in fresh_att_subset.reset_index().iterrows():
                                ...
                                # DRIVE LOGIC
                                sig = str(row.get('SignatureBase64')).strip()
                                if len(sig) > 10: # Assuming it's a File ID now
                                    img_bytes = download_image_from_drive(drive_service, sig)
                                    if img_bytes:
                                        img_bytes.seek(0)
                    ...
                    if pdf_key in st.session_state.pdf_cache:
                         ...
                         st.download_button("📥 Download PDF", st.session_state.pdf_cache[pdf_key], fname, "application/pdf")
    ...
    # --- 3. EMPLOYEE MASTER (KEPT IDENTICAL) ---
...
# ------------------------------------------------------------------------------
# ROUTE C: ACCESS DENIED
# ------------------------------------------------------------------------------
else:
    # --- MODIFIED: USE PASSWORD INPUT IF NO URL PARAM ---
    st.error("⛔ Access Denied.")
    entered_pass = st.text_input("Enter Admin Password", type="password")
    if st.button("Login"):
        if entered_pass == ADMIN_KEY:
            st.session_state.is_admin = True
            st.rerun()
        else:
            st.error("Incorrect.")
